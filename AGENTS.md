...
## 2026-09-09 两个核心目标与下一轮验收

用户明确仅有两个高层目标：**无论 AI 大模型如何进化，都能继续提升项目；最终形成 SAO 完全由 AI 驱动的世界。** 最新裁决与三方交叉证据见[两目标审计](ThreeHearthsVillage/Docs/Design/Two_Goal_Alignment_Audit_2026-09-09.md)。后续实现与验收以这两项目标判断优先级。

下一轮优先验证两件事：A，真实第二模型在保留身份、经历、关系、产权和承诺的同一世界中继续生活，并比较行为与成本；B，居民自主提出的真实能力缺口经过开发、验证、版本化安装和使用回访，且另一名居民能够复用，无需人物专属分支。协议假响应只能作为技术测试，拒绝分支不能替代安装使用成功。目标、需求和结果应可持续保存、检索和更新。

保留现有持久世界、本人信息边界、位置/材料/钱物核验和预算保护。美术服务真实使用与可读性；暂缓无真实需求支持的重复资产和全城精修，避免将这次调整扩大成通用平台重写。货架/箱子只是例子，不得强迫居民提出指定需求。已结束的夜间运行保持截止状态，实际运行安排依据后续用户授权。
...
- 当前世界方向为 SAO 第一层，按上文两个核心目标推进居民自主生活、社会关系、生产交易、探索与世界扩展。旧国王征税路线仅作档案；构件和材质继续分层复用、组合，并以实际居民需求与可见使用结果验收。
