extends SceneTree
## Run the reviewed candidate at its existing world anchor, retaining Tolbana.

func _initialize() -> void:
    call_deferred("launch_demo")

func launch_demo() -> void:
    var candidate = JSON.parse_string(FileAccess.get_file_as_string("res://assets/reference_scenes/market_v3_manifest.json"))
    ...
    var folder := "res://validation/market_demo_v3"
