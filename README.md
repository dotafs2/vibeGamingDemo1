...
- 白盒角色只负责移动、跳跃、推绞盘箱、安装装置、操作装置和切换时间
- 03号闸门、机械绞盘、电磁锁和配重机构负责验证完整的可见因果链
...
- 固定二维正交视角
- 一个受重力约束的白盒角色
- 一个可以推动、也可以作为落脚点的机械绞盘箱
- 一扇在两个时间点位置与编号完全相同的03号闸门
- 一组可以在2047年永久安装并保留到2147年的机械绞盘、链条和配重机构
...
1. 玩家出生在固定的2147年，看见同一扇03号闸门因断电而锁死，门边的手动接口为空。
2. 玩家按 `Q` 前往唯一可达的过去时间点——固定的2047年；03号闸门仍因通电的电磁锁关闭。
3. 玩家把维修区的机械绞盘箱推到闸门接口，按 `E` 永久安装绞盘、链条与配重连接。
4. 2047年的大门不会因此打开，因为通电状态下电磁锁拥有优先控制权，玩家不能从过去绕路。
5. 玩家按 `Q` 返回固定的2147年；同一套绞盘已经老化，但仍安装在同一位置。
6. 玩家按 `E` 转动机械绞盘。断电的电磁锁不再阻碍机械机构，配重拉起大门，安全卡扣将其锁定。
7. 玩家穿过原本无法通过的现代矿道并抵达时间出口。
...
```js
worldHistory.winchInstalled = true;
```
...
- 2047年必须把绞盘推到接口并按 `E` 永久安装，单纯按 `Q` 不会改变2147年
- 安装后2047年的闸门仍被通电电磁锁关闭；2147年则出现同一套老化绞盘
- 2147年必须再次按 `E` 操作机械装置，大门才会实际升起并允许通行
- 玩家能够通过改变历史打开新的场景路径
- 玩家第一眼能理解“过去的行为改变了现代”
