...
1. 玩家出生在现代，看到一扇无法通过的封锁门。
2. 玩家按 `Q` 无缝进入温暖、完整的过去矿镇。
3. 玩家推动带时间标记的箱子，将它压上压力板并锁定历史事件。
4. 玩家按 `Q` 回到冰冷、破败的现代。
5. 现代封锁门受到历史影响，在玩家眼前坍塌。
...
- 过去把时间箱压上压力板后，现代场景会产生明确变化
- 现代封锁门具有可读的坍塌过程，而不是简单隐藏模型
- 玩家能够通过改变历史打开新的场景路径
- 玩家第一眼能理解“过去的行为改变了现代”
