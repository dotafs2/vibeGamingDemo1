...
- 一套由未来NPC预先交给主角的时锁爆破器与微型维修铣刀；初始位于背包第4格，被主角暗装进2047年的门框锁轨后立即从背包消失
- 四种不消耗弹药的试验武器：可自由瞄准的时相校准器、斜向劈砍的磁弧切割刀、蓄力砸落的液压震击锤，以及投出后自动返回的检修扳手
- 武器只在攻击及短暂战斗准备时间内拿在手中；待机时校准器挂回腰侧、切割刀进入刀鞘、震击锤背到身后、扳手扣回腰间
...
- Boss会在平台下方来回行走，瞄准射线时停步支撑；Boss机体没有玩家碰撞体积，主角可以直接穿过，唯一伤害来源是先显示瞄准线、再正式发射的时晶扫描射线
...
- `E` 拆卸、拾取、安装或使用场景装置
- `B` 打开或关闭时间锚背包
- `1 / 2 / 5 / 6` 分别装备时相校准器、磁弧切割刀、液压震击锤和回弹检修扳手
...
18. 玩家越过倒塌门体后，掘脉者识别到时间锚并解除封存。它会用四足在房间底层来回行走，停步后发射测绘射线。玩家沿左右三级维护平台跳跃，躲开预警后的真实射线，再用时相校准器射击中央外露核心；主角可直接穿过机体，接触不阻挡也不受伤。
