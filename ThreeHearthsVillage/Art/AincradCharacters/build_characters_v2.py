...
class Geo:
    ...
    def panel(self,verts,faces,role,bone,thickness=.003):
        # Clothes have front-facing normals toward Blender -Y and real back/edge faces.
        # Blender's former double-sided display hid reversed apron/collar winding in UE.
        front=[];edges={}
        for face in faces:
            face=tuple(face)
            normal=(Vector(verts[face[1]])-Vector(verts[face[0]])).cross(Vector(verts[face[2]])-Vector(verts[face[0]]))
            if normal.y>0:face=tuple(reversed(face))
            front.append(face)
            for a,b in zip(face,face[1:]+face[:1]):
                key=tuple(sorted((a,b)))
                edges.setdefault(key,[]).append((a,b))
        n=len(verts);back=[(x,y+thickness,z) for x,y,z in verts]
        closed=front+[tuple(n+i for i in reversed(f)) for f in front]
        for pairs in edges.values():
            if len(pairs)==1:
                a,b=pairs[0];closed.append((a,a+n,b+n,b))
        self.mesh(list(verts)+back,closed,role,bone)
    ...
    def sweep(self,rows,role,n=20):
        verts=[];weights=[]
        for i,(pos,rx,ry,bones) in enumerate(rows):
            center=Vector(pos)
            ...
            for k in range(n):
                angle=k*2*math.pi/n;verts.append(center+u*rx*math.cos(angle)+v*ry*math.sin(angle))
                weights.append(bones if isinstance(bones,dict) else {bones:1.0})
        ...
        first=len(self.v);self.mesh(verts,faces,role,'root')
        self.weights[first:]=weights

HEIGHT_SCALE={"Aileen":.985,"Takuma":1.038,"Kashiwagi":1.0}
...
def rig(name):
    ...
    factor=HEIGHT_SCALE[name]
    for name,a,b,parent in rows:
        bone=data.edit_bones.new(name);bone.head=Vector(a)*factor;bone.tail=Vector(b)*factor
        if parent:bone.parent=data.edit_bones[parent]
    bpy.ops.object.mode_set(mode='OBJECT');obj.select_set(False)
    return obj

def garment_weights(x, z):
    # Shared cloth/panel weights keep layered surfaces together while each leg advances.
    influence=max(0.0,min(1.0,(.99-z)/.21))
    right=max(0.0,min(1.0,(x+.065)/.13))
    shin=max(0.0,min(.65,(.56-z)/.22*.65))
    weights={'pelvis':1-influence}
    for label,side in [('L',1-right),('R',right)]:
        weights['thigh.'+label]=influence*side*(1-shin)
        weights['shin.'+label]=influence*side*shin
    return {bone:value for bone,value in weights.items() if value>1e-6}

def weight_garment(g, start):
    g.weights[start:]=[garment_weights(v[0],v[2]) for v in g.v[start:]]
...
def character(name):
    ...
    jaw=1.14 if name=='Takuma' else .96 if name=='Kashiwagi' else 1.0
    profile=[(1.49,.018,.022,-.025),(1.515,.047,.042,-.016),(1.55,.078,.064,-.004),(1.59,.102,.080,0),(1.64,.106,.085,.008),(1.70,.092,.078,.013),(1.735,.06,.052,.012),(1.751,.007,.012,.01)]
    g.rings([(z,xr*(jaw if z<1.6 else 1),yr,y) for z,xr,yr,y in profile],'Skin','head',32)
    for side in (-1,1):g.ellipsoid((side*.102,.005,1.587),(.019,.014,.032),'Skin','head',8,12)
    # Curved facial decals hug the real cheek/forehead surface, avoiding floating eyeballs.
    def face_front(x, z, offset):
        for lo,hi in zip(profile,profile[1:]):
            if lo[0] <= z <= hi[0]:
                t=(z-lo[0])/(hi[0]-lo[0])
                xr=lo[1]*(1-t)+hi[1]*t
                if z<1.6: xr*=jaw
                yr=lo[2]*(1-t)+hi[2]*t; cy=lo[3]*(1-t)+hi[3]*t
                return cy-yr*math.sqrt(max(0,1-(x/xr)**2))-offset
        raise ValueError(('face patch outside profile',x,z))
    def face_patch(x,z,rx,upper,lower,role,offset):
        verts=[(x,face_front(x,z,offset),z)]
        for i in range(32):
            a=i/32*2*math.pi; px=x+rx*math.cos(a)
            pz=z+(upper if math.sin(a)>=0 else lower)*math.sin(a)
            verts.append((px,face_front(px,pz,offset),pz))
        g.panel(verts,[(0,1+i,1+(i+1)%32) for i in range(32)],role,'head',.00025)
    for side in (-1,1):
        x=side*.043;z=1.607
        width=.029 if female else .027
        upper=.017 if female else .0145;lower=.012 if female else .0105
        face_patch(x,z,width,upper,lower,'White',.0012)
        face_patch(x,z,.0108,.0135,.0110,'Ink',.0017)
        face_patch(x,z,.0093,.0120,.0100,'Iris',.0021)
        face_patch(x,z,.0046,.0090,.0085,'Ink',.0025)
        face_patch(x-.0032,z+.0044,.0025,.0031,.0031,'White',.0029)
        points=[]
        for i in range(13):
            angle=i/12*math.pi;px=x+width*math.cos(angle);pz=z+upper*math.sin(angle)
            points.append((px,face_front(px,pz,.0018),pz))
        g.stroke(points,.0016,'Ink','head')
        brow=[]
        for dx,pz in [(-.024,1.639),(0,1.645 if female else 1.642),(.024,1.638)]:
            px=x+dx;brow.append((px,face_front(px,pz,.0015),pz))
        g.stroke(brow,.0022 if female else .0027,'Hair','head')
    ...
    for side,label in [(-1,'L'),(1,'R')]:
        ...
        g.sweep([
            ((side*.125,0,1.365),.053,.071,'chest'),
            ((side*.175,0,1.355),.069,.076,{'chest':.45,upper:.55}),
            ((side*.23,0,1.315),.066,.069,upper),
            ((side*.29,0,1.25),.059,.061,upper),
            ((side*.34,0,1.185),.050,.054,{upper:.7,fore:.3}),
            (elbow,.047,.051,{upper:.4,fore:.6}),
            (tuple(Vector(elbow).lerp(Vector(wrist),.11)),.049,.053,fore)],'Cloth')
        cuff=Vector(elbow).lerp(Vector(wrist),.14)
        g.tube(tuple(Vector(elbow).lerp(Vector(wrist),.08)),cuff,.054,.052,'Cloth',fore,20)
        g.sweep([(elbow,.043,.038,{upper:.25,fore:.75}),
            (tuple(Vector(elbow).lerp(Vector(wrist),.32)),.040,.035,fore),
            (tuple(Vector(elbow).lerp(Vector(wrist),.75)),.030,.025,fore),
            (wrist,.023,.020,{fore:.7,hand:.3})],'Skin',20)
        ...
        trouser='Accent' if female else 'Cloth'
        g.sweep([(hip,.077,.084,{'pelvis':.25,thigh:.75}),
            ((side*.10,-.006,.80),.080,.078,thigh),
            ((side*.10,-.017,.63),.061,.056,thigh),
            (knee,.048,.049,{thigh:.5,shin:.5}),
            ((side*.10,-.019,.42),.047,.043,shin),
            ((side*.10,-.008,.29),.040,.039,shin),
            (ankle,.029,.030,shin)],trouser,20)
        g.tube((side*.1,0,.28),(side*.10,0,.072),.047,.044,'Boot',shin,20)
    ...
    for side in (-1,1):
        g.panel([(side*.045,-.059,1.43),(side*.107,-.079,1.37),(side*.050,-.105,1.30),(0,-.101,1.355)],[(0,1,2,3)],'Cloth','chest')
    ...
    if female:
        cloth_start=len(g.v)
        g.rings([(.36,.255,.145,0),(.46,.247,.140,0),(.56,.235,.132,0),(.67,.214,.124,0),(.78,.195,.116,0),(.90,.180,.106,0),(1.01,.145,.095,0)],'Accent','pelvis',48)
        weight_garment(g,cloth_start)
        # Front apron follows the outside of the skirt; no exposed underside.
        verts=[]
        apron_rows=[(1.02,.11,-.103),(.91,.123,-.117),(.80,.135,-.130),(.70,.15,-.137),(.60,.165,-.145),(.50,.175,-.154),(.40,.18,-.161)]
        for z,w,y in apron_rows:
            for i in range(9):
                x=(i/8*2-1)*w;verts.append((x,y+.023*(x/w)**2,z))
        cloth_start=len(g.v)
        g.panel(verts,[(r*9+i,r*9+i+1,(r+1)*9+i+1,(r+1)*9+i) for r in range(len(apron_rows)-1) for i in range(8)],'Cloth','pelvis')
        weight_garment(g,cloth_start)
    else:
        width=.13 if name=='Takuma' else .105
        g.panel([(-width,-.101,1.31),(width,-.101,1.31),(width*.8,-.105,1.05),(-width*.8,-.105,1.05)],[(0,1,2,3)],'Leather','chest')
        apron_rows=[(1.04,.12),(.97,.128),(.90,.136),(.83,.144),(.76,.152),(.70,.16)]
        apron_verts=[((i/8*2-1)*width,-.128-.012*(1.04-z)/.34,z) for z,width in apron_rows for i in range(9)]
        cloth_start=len(g.v)
        g.panel(apron_verts,[(r*9+i,r*9+i+1,(r+1)*9+i+1,(r+1)*9+i) for r in range(len(apron_rows)-1) for i in range(8)],'Leather','pelvis')
        weight_garment(g,cloth_start)
    ...
    factor=HEIGHT_SCALE[name]
    g.v=[tuple(Vector(v)*factor) for v in g.v]
    return g
        ...
        for frame in range(1,32,3):
            ...
            if action_name=='Walk':
                for s,label in [(1,'L'),(-1,'R')]:
                    ...
                    armature.pose.bones['thigh.'+label].rotation_euler.x=swing*.24
                    armature.pose.bones['shin.'+label].rotation_euler.x=-max(0,-swing)*.35
...
characters=[];manifest={'front_axis':'Blender -Y; UE orientation requires native visual verification','height_basis':'Per-asset measured mesh bounds in metres','palette':PALETTES,'assets':[]}
for name,colors in PALETTES.items():
    armature=rig(name);g=character(name);mesh=mesh_object(g,name,{**BASE,**colors},armature);actions=animate(armature)
    ...
    armature.animation_data.action=None
    manifest['assets'].append({'id':name,'fbx':path.relative_to(OUT).as_posix(),'vertices':len(g.v),'triangles':sum(len(f)-2 for f in g.f),'bones':len(armature.data.bones),'roles':list(dict.fromkeys(g.roles)),'height_m':max(v[2] for v in g.v)-min(v[2] for v in g.v),'revision':2})
...
manifest['revision']=2
manifest['scope']='Blender art review prototype; not installed in live world; same three existing identities'
...
if opt.render:
    for i,(name,armature,mesh) in enumerate(characters):
        armature.hide_render=False;mesh.hide_render=False;armature.location.x=(i-1)*.92
        armature.animation_data.action=bpy.data.actions[name+'_Idle']
    scene.frame_set(1)
    ...
    camera.location=(3.8,-6,1.8);camera.rotation_euler=(Vector((0,0,.95))-camera.location).to_track_quat('-Z','Y').to_euler()
    camera.data.ortho_scale=3.45;scene.render.filepath=str(OUT/'Previews/characters-three-quarter.png');bpy.ops.render.render(write_still=True)
    bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'AincradCharacters.blend'))
