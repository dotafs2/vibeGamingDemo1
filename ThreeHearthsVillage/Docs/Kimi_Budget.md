...
用户本轮明确授权“你随便用kimi……大概100，你可以都用完”。当前城市验证使用独立、持久的 `kimi-city-validation-2026-09-07.sqlite3`，硬授权100元、可分配95元、保留5元余量，累计最多600次。旧的夜间账本未同步到本机；本次没有重置旧账本或把账户余额当成旧授权余额。初始化后，启动和重启均不会补额度。两种账本的策略指纹分别核验，不能互换。

当前无时间截止（`deadline_utc=0`），仍限制每居民一个在途请求、全局最多10个。已支持实际 UE 导出的512像素PNG看图：每次一个图像，最大512KiB，上游请求最大768KiB，文本上下文仍为24,576字节、最多8条消息。图片中的可见内容与角色持久故事共同进入请求；使用量仍由上游usage按输入/缓存/输出结算。

已授权的城市示例可用项目根目录 `Start-CityDemo.ps1` 启动。脚本使用独立城市存档，打开UE时启动城市预算网关，关闭UE后清理它自己启动的网关。`-LocalOnly` 只运行本地规则。只读检查/手动服务命令为：

```powershell
python Plugins/ThreeHearths/Tools/kimi_gateway.py status --profile city-validation
python Plugins/ThreeHearths/Tools/kimi_gateway.py serve --profile city-validation
```

当前离线预算验收15项、看图接口验收4项通过，包括新增授权与旧账本隔离、累计调用次数重启后仍受限。下面保留早期夜间授权与验收记录，描述的是当时版本，不能用来重建遗失预算。
