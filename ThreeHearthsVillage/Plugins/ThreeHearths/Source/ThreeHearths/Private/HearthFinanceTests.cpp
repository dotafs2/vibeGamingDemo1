...
bool FHearthPrivateHouseFinanceTest::RunTest(const FString&)
{
    ...
    const FHearthPublicProject OriginalPublicProject=V->PublicProject;
    V->PublicProject.Id=Id(); V->PublicProject.Site=EmptySite; V->PublicProject.Status=TEXT("building");
    TestFalse(TEXT("An active public project keeps its selected site reserved"),EmptySite>=0&&V->IsProductionAllowed(0,100+EmptySite*16));
    V->PublicProject.Status=TEXT("completed");
    TestFalse(TEXT("A completed public wall keeps its physical site occupied"),EmptySite>=0&&V->IsProductionAllowed(0,100+EmptySite*16));
    FHearthSite UnrelatedPlot=PrivatePlot; UnrelatedPlot.StableId=Id(); UnrelatedPlot.Position=FVector(700,0,8); UnrelatedPlot.Approach=FVector(500,0,8);
    const int32 UnrelatedSite=V->ProductionSites.Add(UnrelatedPlot);
    TestTrue(TEXT("An unrelated vacant plot remains available after public funds run out"),V->IsProductionAllowed(0,100+UnrelatedSite*16));
    ...
    TestEqual(TEXT("Local policy chooses the executable private plot instead of an unfunded public job"),V->ChooseProductionLocally(0),100+UnrelatedSite*16);
    V->PublicProject=OriginalPublicProject;
