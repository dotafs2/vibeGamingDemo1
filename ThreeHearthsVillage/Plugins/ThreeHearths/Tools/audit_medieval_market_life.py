"""Read-only cold audit for the native MedievalLife MarketLifeKit import.
...
def _source(relative: str) -> Path:
    ...
    if value.suffix.lower() != ".fbx" or not value.is_file():
        raise RuntimeError("Missing MarketLifeKit FBX: " + str(value))
    return value
...
def _bounds(mesh: ue.StaticMesh) -> tuple[list[float], list[float]]:
    ...
    if any(a >= b for a, b in zip(lo, hi)):
        raise RuntimeError("Degenerate MarketLifeKit bounds: " + mesh.get_path_name())
    return lo, hi
...
def _asset_row(row: dict, module: dict, layer: dict, checksum: str, subsystem: object) -> dict:
    ...
    if not mesh_path.startswith(DEST + "/"):
        raise RuntimeError("MarketLifeKit mesh escaped destination: " + mesh_path)
    mesh = ue.load_asset(mesh_path)
    if not isinstance(mesh, ue.StaticMesh):
        raise RuntimeError("MarketLifeKit asset is not a StaticMesh: " + mesh_path)
    source = _source(str(module["fbx"]))
    import_data = _source_filename(mesh)
    if not _same_source(import_data, source):
        raise RuntimeError("MarketLifeKit importData source mismatch: " + mesh_path)
    if row.get("source_sha256") != checksum:
        raise RuntimeError("MarketLifeKit source hash mismatch: " + asset_id)
    settings = mesh.get_editor_property("nanite_settings")
    if bool(settings.get_editor_property("enabled")):
        raise RuntimeError("MarketLifeKit Nanite must be disabled: " + mesh_path)
    slots = mesh.get_editor_property("static_materials")
    if not slots:
        raise RuntimeError("MarketLifeKit layer has no material slots: " + mesh_path)
    material_paths = []
    for slot in slots:
        material = slot.get_editor_property("material_interface")
        if material is None:
            raise RuntimeError("MarketLifeKit layer has an empty material slot: " + mesh_path)
    ...
    if uv_channels < 1 or vertex_count <= 0:
        raise RuntimeError("MarketLifeKit layer lacks UVs/vertices: " + mesh_path)
    lo, hi = _bounds(mesh)
    return {
        "id": asset_id,
        "module_id": module["id"],
...
def main() -> None:
    ...
    try:
        ...
        for module in manifest["modules"]:
            ...
            if error > 0.5:
                raise RuntimeError(f"MarketLifeKit bounds mismatch {module['id']}: {error:.4f} cm")
        ...
        report["limitations"] = ["Static MarketLifeKit geometry only; no runtime commerce behavior tested.",
                                  "No animation or rig verification is applicable."]
    except Exception as exc:
        report.update(status="failed", error=str(exc))
        ue.log_error("[MedievalMarketLifeAudit] " + str(exc))
        raise
    finally:
        REPORT.parent.mkdir(parents=True, exist_ok=True)
        REPORT.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        ue.log("[MedievalMarketLifeAudit] " + report["status"])
