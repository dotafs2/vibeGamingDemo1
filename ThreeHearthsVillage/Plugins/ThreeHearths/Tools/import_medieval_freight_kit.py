"""Import MedievalLife FreightKit FBX layers as native UE StaticMeshes.

The FreightKit manifest is the source of truth. Each FBX is imported with
Interchange DO_NOT_COMBINE, so every declared object/layer remains a separate
StaticMesh under /Game/ThreeHearths/Generated/MedievalLife/FreightKit. Source
...
INDEX = ROOT / "Content" / "ThreeHearths" / "Data" / "MedievalFreightKitCatalog.json"
...
def _safe_source(relative: str) -> Path:
    ...
    if source.suffix.lower() != ".fbx" or not source.is_file():
        raise RuntimeError("Missing FreightKit FBX: " + str(source))
    return source
...
def _mesh_record(mesh: ue.StaticMesh, module: dict, layer: dict, source: Path,
    ...
    if not mesh.get_path_name().startswith(destination + "/"):
        raise RuntimeError("FreightKit mesh escaped module destination: " + mesh.get_path_name())
    if not _same_source(_source_filename(mesh), source):
        raise RuntimeError("FreightKit importData source mismatch: " + mesh.get_path_name())
    _disable_nanite(mesh)
    slots = mesh.get_editor_property("static_materials")
    if not slots:
        raise RuntimeError("FreightKit layer has no material slots: " + mesh.get_path_name())
    materials = []
    for slot in slots:
        material = slot.get_editor_property("material_interface")
        if material is None:
            raise RuntimeError("FreightKit layer has empty material slot: " + mesh.get_path_name())
        materials.append(material.get_path_name())
    bounds = _bounds(mesh)
    if min(bounds["extent_cm"]) <= 0.0:
        raise RuntimeError("FreightKit layer has degenerate bounds: " + mesh.get_path_name())
    ...
    if uv_channels < 1 or vertex_count <= 0:
        raise RuntimeError("FreightKit layer lacks renderable UV/vertices: " + mesh.get_path_name())
    return {
...
def _save_objects(objects: list[object]) -> None:
    ...
    for obj in objects:
        ...
        if not path.startswith(DEST + "/"):
            raise RuntimeError("FreightKit import escaped destination: " + path)
        if not subsystem.save_loaded_asset(obj, False):
            raise RuntimeError("Cannot save FreightKit asset: " + path)
...
def _import_module(module: dict, previous: dict, replace_changed: bool) -> list[dict]:
    ...
    if ue.EditorAssetLibrary.does_directory_exist(destination) and not replacing:
        raise RuntimeError("Existing FreightKit destination requires prior report or replace switch: " + destination)
    pipeline = _pipeline()
    ...
    if not manager.import_asset(destination, manager.create_source_data(str(source)), params):
        raise RuntimeError("Interchange rejected FreightKit module: " + module_id)
    meshes = [obj for obj in imported if isinstance(obj, ue.StaticMesh)]
    if len(meshes) != len(layers):
        raise RuntimeError(f"Expected {len(layers)} FreightKit StaticMeshes for {module_id}, got "
                           f"{[mesh.get_path_name() for mesh in meshes]}")
    by_layer = {}
    for mesh in meshes:
        layer = _layer_for_mesh(mesh, module)
        if layer is None:
            raise RuntimeError("Cannot classify FreightKit imported mesh: " + mesh.get_path_name())
        if layer in by_layer:
            raise RuntimeError("Duplicate FreightKit layer: " + module_id + "/" + layer)
        by_layer[layer] = mesh
    if set(by_layer) != {row["layer"] for row in layers}:
        raise RuntimeError("FreightKit layer set mismatch for " + module_id)
...
def _write_index(manifest: dict, report: dict) -> None:
    ...
    INDEX.write_text(json.dumps({
        ...
        "limitations": ["FreightKit contains static cargo/depot art only; no runtime cart behavior is implied."],
...
def main() -> None:
    ...
    try:
        manifest = _manifest()
        replace_changed = "HearthMedievalFreightKitReplaceChanged" in _switches()
        previous = {}
        if REPORT.is_file():
        ...
        if len(report["assets"]) != len(expected_ids) or actual_ids != expected_ids:
            raise RuntimeError("FreightKit layer asset set mismatch")
        for row in report["assets"]:
            if row.get("source_sha256") != current_hashes.get(row.get("module_id")):
                raise RuntimeError("FreightKit report source hash is stale: " + str(row.get("id")))
        report.update(status="passed", source_manifest_sha256=_sha256(MANIFEST),
                      source_to_unreal_matrix=MATRIX)
        _write_index(manifest, report)
    except Exception as exc:
        report.update(status="failed", error=str(exc))
        ue.log_error("[MedievalFreightKitImport] " + str(exc))
        raise
    finally:
        REPORT.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
