...
下一版的分步实现范围与验收见 [MVP 1 → 2 计划](Docs/MVP_1_to_2.md)。已完成第 1 步，其余步骤按计划分别实施。

第 1 步的手动 PIE 导入预览已在本机 UE 5.8.1 验收，支持本地 GLB 导入、失败回退及退出清理，见 [操作与实拍](Docs/PIE_Import_Preview.md)。当前导入依赖独立编辑器工作进程。
...
历史与世界都保存在本机；结束播放后再次打开会恢复房屋、库存、作物和进行中的任务。“重新开始”会保留旧档并建立新世界。每30秒自动保存，详见[世界存档与恢复](Docs/World_Persistence.md)。扩建住宅目前不会增加人口，日常进食和作物腐烂尚未接入。
