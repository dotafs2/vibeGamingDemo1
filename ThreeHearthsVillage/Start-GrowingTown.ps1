# Development launcher: creates or resumes the separate growing-town world.
# See Docs/Town_Growth_Iteration_Status.md for measured runtime and visual acceptance.
...
try {
    if(-not $LocalOnly) {
        ...
        if($LASTEXITCODE -ne 0){throw 'The existing city-validation budget ledger is unavailable. No budget was reset.'}
        $existing=$null
    ...
    $argsForEditor=@("`"$project`"",'-d3d11','-HearthCityV3','-HearthUnpaused',"`"-HearthWorld=$saved\World\growing-town.json`"","`"-HearthHistory=$saved\growing-town-history.json`"")
    if($LocalOnly){$argsForEditor+='-HearthDisableApi'}
    # This launcher is user-invoked and deliberately opens the interactive editor.
