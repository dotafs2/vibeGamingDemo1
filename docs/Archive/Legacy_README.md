...
- 四种不消耗弹药的试验武器：可自由瞄准的时相枪、缩短钻头并用前后双握把持握的双手脉冲钻、点击一次便从头顶沿肩部圆弧向前劈下并立即收回的液压锤，以及投出后自动返回的开口扳手
- 武器固定占用快捷栏 `1 / 2 / 3 / 4`，机械手轮位于第5格，任务用时锁爆破器固定放在第0格
- 武器只在攻击及短暂战斗准备时间内拿在手中；液压锤完成一次劈砸后立即背回身后，其他武器待机时挂回腰侧或背部
...
- Boss总生命值为1440。一阶段顶部测绘器约每2.5秒完成一次“锁定线—长激光”攻击，只提高释放频率，不改变0.45秒锁定时间、0.2秒激光持续时间或贯穿方式；二阶段彻底取消长激光，改为约每0.42秒发射一枚短激光段。前方锯齿在普通巡逻时没有接触伤害；一阶段每10秒、二阶段每5.6秒进入喷气预警，喷气结束当帧立即冲刺，一阶段速度为26、二阶段为32，只有冲刺阶段会造成伤害
- 二阶段以“约5.6秒短激光弹幕窗口—清除残留光弹—1.25秒喷气预警—锯齿冲刺—快速跳钻”为一轮。计入冲刺与跳钻动作后，两次冲刺开始之间约9至10秒，避免短激光、锯齿和落地钻头同时形成无法读取的伤害叠加
- Boss降至半血后，每次锯齿冲刺结束必定接一次约0.8秒跳钻：0.35秒上升、0.2秒追踪、0.24秒快速下落；起跳期间核心无敌，腹部加宽但缩短的大型金属钻头从舱口直接伸出，不显示多余的黑色伸缩杆
- `S / Ctrl` 下蹲时脚保持着地，髋部下降、双膝弯曲、头和躯干随重心降低，同时缩短碰撞高度以躲过地面锯齿；站在Boss房三级单向平台上时按下蹲会穿过当前平台
- 液压锤采用单手近战使用动画：持锤手臂、手掌与锤柄保持为一个刚性整体，以肩部为固定轴从前上方向前下方旋转一次；另一只手和躯干不参与旋转，完成后立即收回
- 回弹扳手不会在按键瞬间凭空生成投射物：先在手中完成约0.17秒的投掷摆臂，经过身体前方时脱手；离手后以每秒6.2弧度的最大转向速度追踪Boss核心，命中或飞行1.3秒后自动返回
- 武器按风险建立明确输出梯度：手枪24伤害、0.3秒冷却，理论输出约80；双手钻32伤害、0.22秒冷却，理论输出约145；液压锤84伤害、0.52秒冷却，理论输出约162。钻头和锤子必须进入Boss近身范围，因此伤害回报明显高于安全的远程手枪
- 2047年与2147年两个固定时间点；玩家不能前往两者之间的任意年份
- 过去使用红色表现
- 现代使用白色表现
- `A / D` 左右移动
- `W` 跳跃
- `S / Ctrl` 下蹲；站在单向平台上时向下穿过平台
- `E` 拆卸、拾取、安装或使用场景装置
- `B` 打开或关闭时间锚背包
...
18. 玩家越过倒塌门体后，掘脉者识别到时间锚并解除封存。一阶段观察约每2.5秒一次的长激光锁定线；半血后二阶段改为约5.6秒短激光弹幕窗口，随后清场并进入更高速的喷气冲刺，玩家还要在冲刺结束后立刻躲开约0.8秒快速跳钻。主角仍可直接穿过普通巡逻中的机体和锯齿，只有长激光、短激光段、冲刺锯齿和落地钻头会造成伤害。
...
网页右侧的 `EDITOR` 按钮会暂停游戏并进入独立编辑模式。编辑器面向不会美术和技术的使用者，用于把场景逐步替换或补充为双时代高质量透明 2D PNG 组件。

基本流程：

1. 点击“＋ 添加新物体”。
2. 在场景中拖一个框。
3. 在选框旁输入一句自然语言，例如“这里放一台会缓慢旋转的旧工业风扇”。
4. 点击“复制给 Codex”，再到 Codex 中粘贴发送。
5. Codex 会读取自动保存的框选位置、现场截图、2047/2147 参考图和输出路径，按 [`docs/EDITOR_ASSET_WORKFLOW.md`](../../docs/EDITOR_ASSET_WORKFLOW.md) 生成双时代透明 PNG。
6. 网页检测到结果后自动把新组件放回原框位置，并保留旧版本供前后切换。

编辑器默认只显示“移动物体、圈选已有、添加新物体、画风参考、保存并试玩”。精确坐标、宽高、旋转、透明度、前后层级、网格、碰撞框、动画速度和碎裂块数都收在“设置”或“更多调整”中。自然语言会自动判断普通场景物、持续动画或物理组件。物理组件可以被玩家武器打碎，默认分成 4 个由 CPU 更新的简单凸包碎块。

## 本地运行

项目包含本地 Three.js 文件，不依赖 CDN。资产编辑器需要 Vite 的本地保存接口，因此使用项目开发服务器启动：
